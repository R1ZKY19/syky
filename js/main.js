...
const ICONS = {
  ...
  firstdate: 'fa-glass-cheers',
  trip: 'fa-plane',
  firstchat: 'fa-comment',
};
...
function initGate() {
  ...
  if (!cfg.enabled) { gateEl.style.display = 'none';
    body.classList.remove('locked'); return; }

  const savedOk = sessionStorage.getItem('gate_ok') === '1';
  if (savedOk) { gateEl.style.display = 'none';
    body.classList.remove('locked'); return; }
  ...
  async function tryEnter() {
    ...
    if (hash === cfg.passwordHash) {
      ...
      setTimeout(() => { gateEl.style.display = 'none';
        body.classList.remove('locked'); }, 400);
    } else {
...
function initHeroParticles() {
  ...
  document.getElementById('hero-names').innerHTML =
    `${escapeHtml(COUPLE_CONFIG.names.person1)} <span class="heart-icon"><i class="fas fa-heart"></i></span> ${escapeHtml(COUPLE_CONFIG.names.person2)}`;
...
function imgOrPlaceholder(src, alt) {
  return `<img src="${src}" alt="${escapeHtml(alt || '')}" loading="lazy"
    onerror="this.outerHTML='<div class=&quot;photo-placeholder&quot;><i class=&quot;fas fa-image&quot;></i><span>Ganti foto ini di<br><code>config.js</code></span></div>'">`;
}
...
function imgOrPlaceholderSmall(src) {
  return `<img src="${src}" alt="" style="width:100%;height:100%;object-fit:cover" onerror="this.outerHTML='<i class=&quot;fas fa-heart&quot; style=&quot;color:var(--glow);font-size:28px;&quot;></i>'">`;
}
  ...
  grid.innerHTML = withInfo.map(d => {
    ...
    return `
      <div class="date-card reveal visible ${isNearest ? 'nearest' : ''}">
        <div class="d-icon"><i class="fas ${ICONS[d.type] || 'fa-calendar-day'}"></i></div>
...
function initMusicPlayer() {
  ...
  playBtn.addEventListener('click', () => {
    if (audio.paused) {
      audio.play().catch(() => { playBtn.innerHTML = '<i class="fas fa-play"></i>'; });
  ...
  audio.addEventListener('error', () => { document.getElementById('music-artist').textContent = 'Ganti file musik di assets/music/'; });
}
...
function initInteractiveHearts() {
  document.addEventListener('click', e => {
    if (e.target.closest('button, a, input, .photo-card, .filter-chip, .envelope, .secret-btn, .cta-btn, .enter-btn, .icon-btn, .hamburger, .close-menu, .music-play-btn, #admin-toggle')) return;
...
function initAdminPanel() {
  ...
  document.addEventListener('click', e => {
    if (e.target.id === 'admin-panel' || e.target.closest('.admin-close')) { panel.classList.remove('open'); }
  });
}
